...
import CreasePlusCore as crepcore
...
def cPmayaScriptDir():
    return mc.moduleInfo(mn='MS_Toolkit',p=True)+'/tools/MSTools/MST_DATA/scripts/'

def cPiconDir():
    return cPmayaScriptDir() + 'CreasePlusIcons/'

# global_maya_script_dir = mc.internalVar(usd=True)
# global_icons_dir = global_maya_script_dir + 'CreasePlusIcons/'
...
class CreasePlusBtn(QWidget):
    ...
    def __init__(
        ...
        self.btn.setSizePolicy(QSizePolicy.Ignored,
...
def creaseplusclosethefucknwinbefoh(clientobj = None):
    ...
    if mc.window(global_creasePlusMainUi,ex=True):
        # print("hey here!")
        mc.deleteUI(global_creasePlusMainUi, wnd=True)

...
# TODO remove reloads
# crep = reload(crep)
# CreasePlusNodes = reload(CreasePlusNodes)
...
def cPmayaScriptDir():
    return mc.moduleInfo(mn='MY_menu', p=True)+'/scripts/ScriptPackages'
...
def cPhasbevelCage(shpStr):
    ...
    if cpheNode and bevelNode and beveli:
        ...
        if isinstance(plugs, basestring) and mc.nodeType(cPplugNode(plugs)) == 'mesh':
...
def cPisbevelCage(cageshpstr):
    ...
    if isinstance(plugs, basestring) and mc.nodeType(cPplugNode(plugs)) == CreasePlusNodes.CpHeIds.kNodeName:
...
def creasePlusHBevelLiveCmd():
    ...
    bevelNode = None
    if isinstance(srcPlugs, basestring) and mc.nodeType(
    ...
    sourceMesh = None
    if isinstance(srcPlugs, basestring):
        plugStr = srcPlugs

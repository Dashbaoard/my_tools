...
class MainClassBadGeo:
    ...
    def __init__(self):
        ...
        cmds.rowColumnLayout(numberOfRows=1, w=sizeX, parent=mainLayout, rowHeight=[(1, 25), (2, 60)],
                             cs=[(5, 5), (5, 5)])  # 布局   行数    宽度   父布局    行高度   列间距
        cmds.button(label=u"窗口", w=sizeX / 3, h=25, c=self.aaa, align="center",ann="Select objects by name")  # 按钮  宽度   高度   命令    居中     注释
        self.WindowTest = cmds.textField(w=sizeX * 0.2,
                                             ann="Select by Name \n Use * after and/or before the text to select by prefix/suffix \n Example: *_grp")  # 文本控件   宽度    注释
    ...
    def aaa(self, *args):
        ...
        if result == 'OK':
            ...
            self.WindowTest = cmds.textField(self.WindowTest, tx=text, e=True)
